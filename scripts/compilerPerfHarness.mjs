 ...
 * toca la lógica de server/compiler.js, no crea rama, no hace commit, no abre
 * PR.
 *
 * ---------------------------------------------------------------------------
 * QUÉ ES REAL
 * ---------------------------------------------------------------------------
 * Este harness importa los símbolos REALES de server/compiler.js — no hay
 * copias locales. server/compiler.js exporta explícitamente, además de
 * instrumentSource/fileSlugFor: NODE_BUILTINS, ALIAS, routerShimPlugin,
 * virtualFilesPlugin y esmShResolverPlugin. Por eso mide el compilador real,
 * no una copia que se puede desincronizar.
 *
 * El ALIAS del harness es `{ ...ALIAS, ...ALIAS_EXTRA_SOLO_HARNESS }`: el
 * ALIAS real de compiler.js más dos entradas (tslib, iceberg-js) que sólo
 * existen en esta copia local del objeto — nunca en compiler.js — porque el
 * entorno de este harness no tiene salida a esm.sh (ver "CORRECCIÓN v2" más
 ...
 * countedStatSync) se PERDIÓ al dejar de mantener una copia local de
 * virtualFilesPlugin: ya no hay forma de instrumentar líneas dentro de una
 * función importada sin modificar su lógica. El reporte ya no imprime ese
 * desglose (antes, en vez de eliminarlo, hubiera impreso ceros engañosos).
 * Los seis contadores externos (R150/R225/R345/L309/R463/R494), el metafile
 * y la báscula de éxito de cada escenario se miden igual que antes,
 * envolviendo desde fuera el objeto `build` que cada plugin.setup() recibe.
 *
 * ---------------------------------------------------------------------------
...
// ===========================================================================
// ALIAS_EXTRA_SOLO_HARNESS — NO existen en el ALIAS real de compiler.js. Se
// añaden ÚNICAMENTE en esta copia local (spreadeada sobre el ALIAS real más
// abajo) para que el build offline del harness termine con
// result.errors.length === 0 (requisito PASO 0). tslib e iceberg-js son los
// dos paquetes que, según el propio comentario de compiler.js junto a ALIAS,
// NO están vendorizados en el ALIAS real y siguen resolviendo por esm.sh —
// auth-js/functions-js importan 'tslib', storage-js importa 'iceberg-js'.
// Entries verificadas contra el campo "module"/"exports" de cada
// package.json (PASO 0):
//   tslib/package.json        → exports['.'].module = './tslib.es6.mjs'
//   iceberg-js/package.json   → exports['.'].import  = './dist/index.mjs'
// ===========================================================================
const ALIAS_EXTRA_SOLO_HARNESS = {
...
// .mjs pre-empaquetado. Devuelve las métricas de los 6 puntos, el metafile
// resumido, result.errors.length y los bytes de salida JS/CSS (requisito A).
// ===========================================================================
